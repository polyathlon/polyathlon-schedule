...
jQuery(window).load(function () {
    ...
    if(poly_action == 'edit'){
        poly_schedule = polyAjaxGetWithId(poly_schedule.id, 'poly_get_age_groups');
    ...
    jQuery("#poly-save-schedule-button").on( 'click', function( evt ){
        ...
        if(!poly_schedule.title){
            alert(<?php echo __("Oops! You're trying to save a age group without name.", "schedule")?>);
            return;
        }
        ...
        //Perform Ajax calls
        poly_result = polyAjaxSave(poly_schedule, 'poly_save_age_groups');
