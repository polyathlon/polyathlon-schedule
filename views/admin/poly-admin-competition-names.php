...
jQuery(window).load(function () {
    ...
    if(poly_action == 'edit'){
        poly_schedule = polyAjaxGetWithId(poly_schedule.id, 'poly_get_competition_names');
    ...
    jQuery("#poly-save-schedule-button").on( 'click', function( evt ){
        ...
        if(!poly_schedule.name){
            alert("<?php echo __("Oops! You're trying to save a competition name without name.", "schedule")?>");
            return;
        }
        ...
        //Perform Ajax calls
        poly_result = polyAjaxSave(poly_schedule, 'poly_save_competition_names');

        //Get updated model from the server
        poly_schedule = polyAjaxGetWithId(poly_result['pid'], 'poly_get_competition_names');
        poly_schedule = validatedSchedule(poly_schedule);
        poly_schedule.isDraft = false;
