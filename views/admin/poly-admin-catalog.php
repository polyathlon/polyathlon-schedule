...
<div class="poly-schedule-header">
    ...
    <div class="poly-three-parts poly-fl poly-title-part"><input id="poly-schedule-title" class="poly-schedule-title" name="schedule-title" maxlength="250" placeholder="Enter catalog title" type="text"></div>
...
<div class="poly-project-wrapper poly-catalog-wrapper">
    <aside class="poly-project-sidebar">
        <div>
            <a id="poly-add-project-button" class='button-secondary poly-add-project-button poly-glazzed-btn poly-glazzed-btn-green' href='#' title='Add new'>+ New product</a>
        </div>
        ...
        <div class="poly-project-details-wrapper">
            <aside class="poly-project-details-sidebar">
                <div id="poly-project-details-content">
                    <input id="poly-project-title" class="poly-project-title" name="project.title" value="" type="text" placeholder="Enter product name">
                    ...
                    <textarea id="poly-project-description" name="project.description" placeholder="Enter product description..."></textarea>
                    <input id="poly-project-url" name="project.url" value="" type="text" placeholder="Enter checkout url">
                    <input class="poly-fullwidth-input" id="poly-project-price" name="project.price" value="" type="text" placeholder="Enter product price">
                    <input class="poly-fullwidth-input" id="poly-project-sale" name="project.sale" value="" type="text" placeholder="Enter product sale (%)">
                </div>
            </aside>
    ...
    if(poly_schedule.projects && poly_schedule.corder){
        for(var poly_projectIndex = 0; poly_projectIndex < poly_schedule.corder.length; poly_projectIndex++){
            ...
            if(poly_project.id == poly_selectedProjectId){
                ...
                if (poly_project.details && typeof poly_project.details.price != 'undefined') {
                    jQuery("#poly-project-price").val( (poly_project.details.price ? poly_project.details.price : '') );
                } else {
                    jQuery("#poly-project-price").val( "");
                }
                if (poly_project.details && typeof poly_project.details.sale != 'undefined') {
                    jQuery("#poly-project-sale").val( (poly_project.details.sale ? poly_project.details.sale : '') );
                } else {
                    jQuery("#poly-project-sale").val( "");
                }
...
function poly_updateModel(){
  //To make sure it's valid JS object
    ...
    poly_schedule.extoptions = {
        all_cats: {},
        type: '<?php echo POLYGridType::CATALOG; ?>'
    };
    ...
    if(poly_selectedProjectId){
        ...
        var details = {
            price: jQuery("#poly-project-price").val(),
            sale: jQuery("#poly-project-sale").val(),
            status: '<?php echo POLYProductStatus::Visible; ?>'
        };
        poly_activeProject.details = details;
...
function onScheduleOptions(){
    if(poly_schedule.isDraft){
        alert("Save the draft schedule before changing the view options");
    }else{
