...
<div class="poly-schedule-header">
    ...
    <input id="poly-schedule-competition-name" class="poly-schedule-title" name="schedule-title" maxlength="250" placeholder="<?php echo __("Enter competition name", "schedule")?>"  type="text"></div>
...
jQuery(window).load(function () {
    ...
    if(poly_action == 'edit'){
        poly_schedule = polyAjaxGetWithId(poly_schedule.id, 'poly_get_competition_stages');
    ...
    jQuery("#poly-save-schedule-button").on( 'click', function( evt ){
        ...
        if(!poly_schedule.title){
            alert(<?php echo __("Oops! You're trying to save a competition stages without name.", "schedule")?>);
            return;
        }
        ...
        //Perform Ajax calls
        poly_result = polyAjaxSave(poly_schedule, 'poly_save_competition_stages');

        //Get updated model from the server
        poly_schedule = polyAjaxGetWithId(poly_result['pid']);
        poly_schedule = validatedSchedule(poly_schedule);
        poly_schedule.isDraft = false;
