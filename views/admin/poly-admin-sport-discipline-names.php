...
<div class="poly-schedule-header">
    ...
    <div class="poly-three-parts poly-fl poly-title-part">
        <input id="poly-schedule-name" class="poly-schedule-name" name="schedule-title" maxlength="250" placeholder= "<?php echo __("Enter discipline name", "schedule")?>" type="text">
    </div>
...
<div class="poly-details-sidebar">
    <input id="poly-schedule-abbreviated-name" class="poly-schedule-name" name="schedule-abbreviated-name" maxlength="250" placeholder= <?php echo __("Enter abbreviated name", "schedule")?> type="text">
</div>
...
jQuery(window).load(function () {
    ...
    if(poly_action == 'edit'){
        poly_schedule = polyAjaxGetWithId(poly_schedule.id, 'poly_get_sport_discipline_names');
    ...
    jQuery("#poly-save-schedule-button").on( 'click', function( evt ){
        ...
        if(!poly_schedule.name){
            alert("<?php echo __("Oops! You're trying to save a sport discipline name without name.", "schedule")?>");
            return;
        }
        ...
        //Perform Ajax calls
        poly_result = polyAjaxSave(poly_schedule, 'poly_save_sport_discipline_names');

        //Get updated model from the server
        poly_schedule = polyAjaxGetWithId(poly_result['pid'], 'poly_get_sport_discipline_names');
        poly_schedule = validatedSchedule(poly_schedule);
        poly_schedule.isDraft = false;
...
function poly_updateUI(){
    ...
    if(poly_schedule.name){
        jQuery("#poly-schedule-abbreviated-name").val( poly_schedule.abbreviated_name );
    }
...
function poly_updateModel(){
  //To make sure it's valid JS object
    ...
    poly_schedule.abbreviated_name = jQuery("#poly-schedule-abbreviated-name").val();
}
