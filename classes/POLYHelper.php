...
class POLYHelper {
    ...
    static public function tcButtonShortcodes(){
        ...
        $results = $wpdb->get_results( "SELECT * FROM ".POLY_TABLE_SCHEDULES , ARRAY_A );
        ...
        for($i=0; $i<count($results); $i++){
            ...
            $shortcode->shortcode = POLYHelper::shortcodeWithPID($results[$i]["id"]);
    ...
    static public function getScheduleWithId($pid){
        if(!$pid) return null;
        ...
        if(count($res)) {
            ...
            $query = $wpdb->prepare(
                "SELECT ". POLY_TABLE_SCHEDULE_ITEMS .".*, ".
                        POLY_TABLE_COMPETITION_NAMES. ".name AS competition_name FROM ". POLY_TABLE_SCHEDULE_ITEMS. " LEFT OUTER JOIN ". POLY_TABLE_COMPETITION_NAMES." ON ".
                POLY_TABLE_SCHEDULE_ITEMS.".".POLY_TABLE_COMPETITION_NAMES_ID." = ".
                POLY_TABLE_COMPETITION_NAMES.".".POLY_TABLE_COMPETITION_NAMES_ID." WHERE ".POLY_TABLE_SCHEDULE_ITEMS.".".POLY_TABLE_COMPETITION_NAMES_ID." = %d", $pid);
            ...
            //   }else{
            //       $schedule->options = json_decode( base64_decode(POLYHelper::getScheduleDefaultOptions(0, $gridType)), true);
            //   }
            //   if(!empty($schedule->extoptions)){
            //       $schedule->extoptions = json_decode($schedule->extoptions, true);
            //   } else {
            //       $schedule->extoptions = array('type' => POLYGridType::ALBUM);
            //   }
        }
    ...
    static public function formatDate($start_date, $end_date) {
        ...
        return $startMonth != $endMonth? $startDay.' '.$months[$startMonth].' - '.$endDay.' '.$months[$endMonth]
            : ($startDay != $endDay ? $startDay.'-'.$endDay.' '.$months[$startMonth]
            : $startDay.' '.$months[$startMonth]);
    }

    static public function StrToStartDate($date) {
        $months = array('Января' => '01', 'Февраля' => '02', 'Марта' => '03', 4 => 'Апреля',
    ...
    static public function uniqueCompetitionNames($array) {
        $temp_array = [];
        foreach ($array as &$v) {
            if (!isset($temp_array[$v->competition_name]))
            $temp_array[$v->competition_name] = $v->competition_name;
        }
        return $temp_array;
    }


    static public function getScheduleItems($pid){
        if( !$pid ) return null;
        ...
        $query = $wpdb->prepare(
            "SELECT ". POLY_TABLE_SCHEDULE_ITEMS .".*, ".
                    POLY_TABLE_COMPETITION_NAMES. ".name AS competition_name, ".
                    POLY_TABLE_CITIES. ".name AS city_name, ".
                    POLY_TABLE_STAGES. ".name AS stage_name, ".
                    POLY_TABLE_COUNTRIES. ".name AS country_name ".
                    " FROM ". POLY_TABLE_SCHEDULE_ITEMS.
                    " LEFT OUTER JOIN ". POLY_TABLE_COMPETITION_NAMES." ON ".
                        POLY_TABLE_SCHEDULE_ITEMS.".".POLY_TABLE_COMPETITION_NAMES_ID." = ".
                        POLY_TABLE_COMPETITION_NAMES.".".POLY_TABLE_COMPETITION_NAMES_ID.
                    " LEFT OUTER JOIN ".POLY_TABLE_CITIES." ON ".
                        POLY_TABLE_SCHEDULE_ITEMS.".".POLY_TABLE_CITIES_ID." = ".
                        POLY_TABLE_CITIES.".".POLY_TABLE_CITIES_ID.
                    " LEFT OUTER JOIN ".POLY_TABLE_COUNTRIES." ON ".
                        POLY_TABLE_CITIES.".".POLY_TABLE_COUNTRIES_ID." = ".
                        POLY_TABLE_COUNTRIES.".".POLY_TABLE_COUNTRIES_ID.
                    " LEFT OUTER JOIN ".POLY_TABLE_STAGES." ON ".
                        POLY_TABLE_SCHEDULE_ITEMS.".".POLY_TABLE_STAGES_ID." = ".
                        POLY_TABLE_STAGES.".".POLY_TABLE_STAGES_ID.
                    " WHERE ".POLY_TABLE_SCHEDULE_ITEMS.".".POLY_TABLE_SCHEDULES_ID." = %d", $pid);

        $scheduleItems = $wpdb->get_results( $query , OBJECT );

        if( count($scheduleItems) ){
            foreach( $scheduleItems as $scheduleItem ){
                $scheduleItem->pic = json_decode( base64_decode( $scheduleItem->image ) );
                $scheduleItem->date = POLYHelper::formatDate($scheduleItem->start_date, $scheduleItem->end_date);
                $scheduleItem->ageGroups = json_decode($scheduleItem->age_groups, true);
            }
        }
        return $scheduleItems;
    }

    static public function getScheduleDefaultOptions($pid = 0, $gridType = POLYGridType::ALBUM){
        //TODO: Update this each time if any default option is changed

        if ($gridType == POLYGridType::CLIENT_LOGOS) {
            $options = '{"id":"' . $pid . '","kThumbnailQuality":"large","kShowCategoryFilters":false,"kShowCaptionTitle":false,"kShowCaptionSubtitle":false,"kDisableAlbumStylePresentation":true,"kEnablePictureCaptions":false,"kExcludeCoverPicture":true,"kLinkIcon":"link","kZoomIcon":"search","kGoIcon":"","kLayoutWidth":"100","kLayoutWidthUnit":"%","kTileApproxWidth":"200","kTileApproxHeight":"150","kTileMinWidth":"150","kTileMargins":"4","kLayoutAlignment":"center","kTintColor":"#000000","kProgressColor":"#5c9b30","kFiltersColor":"#969696","kFiltersHoverColor":"#1e73be","kPaginationColor":"#969696","kPaginationHoverColor":"#1e73be","kTileTitleColor":"#ffffff","kTileTitleHoverColor":"#eeee22","kTileDescColor":"#ffffff","kTileDescHoverColor":"#ffffff","kTileOverlayColor":"#000000","kTileOverlayOpacity":"80","kTileIconsColor":"#000000","kTileIconsBgColor":"#ffffff","kTileIconsBgOpacity":"CC","kTileIconsBgHoverColor":"#dd9933","kTileIconsBgHoverOpacity":"E6","kDirectLinking":true,"kLoadUrlBlank":true,"kGoogleMapApiKey":"","kEnableGridLazyLoad":false,"kEnableCategoryAjaxLoad":true,"kCustomJS":"","kCustomCSS":"","kLayoutType":"3","kViewerType":"2","kTileCaptionType":"btn01","kMouseType":"pointer","kItemsPerPage":"' . POLYOption::kItemsPerPageDefault . '","kEnablePagination":false,"kMaxVisiblePageNumbers":"' . POLYOption::kMaxVisiblePageNumbersDefault . '","kPaginationAlignment":"right","kPaginationStyle":"poly-pagination-style-1","kDescMaxLength":"15","kTileTitleFontSize":"18","kTileDescFontSize":"11","kTileTitleAlignment":"center","kTileDescAlignment":"center","kShowTitle":false,"kShowDesc":true,"kShowOverlay":false,"kShowLinkButton":true,"kShowExploreButton":false,"kShowFacebookButton":false,"kShowTwitterButton":false,"kShowGooglePlusButton":false,"kShowPinterestButton":false,"kHideAllCategoryFilter":true,"kAllCategoryAlias":"All","kFilterStyle":"poly-filter-style-1","kDetailsDisplayStyle":"details27","kPictureHoverEffect":"image06","kOverlayDisplayStyle":"overlay00","kOverlayButtonsDisplayStyle":"button05","kShareButtonsDisplayStyle":"share20","kOverlayButtonsHoverEffect":""}';
        } elseif ($gridType == POLYGridType::TEAM) {
            $options = '{"id":"' . $pid . '","kThumbnailQuality":"large","kShowCategoryFilters":false,"kShowCaptionTitle":false,"kShowCaptionSubtitle":false,"kDisableAlbumStylePresentation":true,"kEnablePictureCaptions":false,"kExcludeCoverPicture":true,"kLinkIcon":"link","kZoomIcon":"search","kGoIcon":"","kLayoutWidth":"100","kLayoutWidthUnit":"%","kTileApproxWidth":"200","kTileApproxHeight":"300","kTileMinWidth":"170","kTileMargins":"4","kLayoutAlignment":"center","kTintColor":"#000000","kProgressColor":"#5c9b30","kFiltersColor":"#969696","kFiltersHoverColor":"#1e73be","kPaginationColor":"#969696","kPaginationHoverColor":"#1e73be","kTileTitleColor":"#ffffff","kTileTitleHoverColor":"#eeee22","kTileDescColor":"#ffffff","kTileDescHoverColor":"#ffffff","kTileOverlayColor":"#000000","kTileOverlayOpacity":"80","kTileIconsColor":"#000000","kTileIconsBgColor":"#ffffff","kTileIconsBgOpacity":"CC","kTileIconsBgHoverColor":"#dd9933","kTileIconsBgHoverOpacity":"E6","kDirectLinking":false,"kLoadUrlBlank":true,"kGoogleMapApiKey":"","kEnableGridLazyLoad":false,"kEnableCategoryAjaxLoad":true,"kCustomJS":"","kCustomCSS":"","kLayoutType":"3","kViewerType":"2","kTileCaptionType":"btn01","kMouseType":"pointer","kItemsPerPage":"' . POLYOption::kItemsPerPageDefault . '","kEnablePagination":false,"kMaxVisiblePageNumbers":"' . POLYOption::kMaxVisiblePageNumbersDefault . '","kPaginationAlignment":"right","kPaginationStyle":"poly-pagination-style-1","kDescMaxLength":"15","kTileTitleFontSize":"18","kTileDescFontSize":"11","kTileTitleAlignment":"right","kTileDescAlignment":"right","kShowTitle":true,"kShowDesc":true,"kShowOverlay":true,"kShowLinkButton":true,"kShowFacebookLink":true,"kShowLinkedinLink":true,"kShowExploreButton":false,"kShowFacebookButton":false,"kShowTwitterButton":false,"kShowGooglePlusButton":false,"kShowPinterestButton":false,"kHideAllCategoryFilter":true,"kAllCategoryAlias":"All","kFilterStyle":"poly-filter-style-1","kDetailsDisplayStyle":"details03","kPictureHoverEffect":"image05","kOverlayDisplayStyle":"overlay00","kOverlayButtonsDisplayStyle":"button06","kShareButtonsDisplayStyle":"share20","kOverlayButtonsHoverEffect":""}';
        } elseif ($gridType == POLYGridType::SLIDER) {
    ...
    {
        if (is_null(POLYHelper::$banners)) {
            $lastLoadedAt = get_option(POLY_BANNERS_LAST_LOADED_AT);
            $lastLoadedAt = !empty($lastLoadedAt) ? $lastLoadedAt : 0;
            if ($lastLoadedAt == 0 || time() - $lastLoadedAt > 1 * 24 * 60 * 60) {
                $result = POLYHelper::sendApiRequest(array(
                    'pkg' => POLY_LICENSE_TYPE,
                    'action' => 'get_banners'
                ), 'GET');
                $cache = array();
                if (!empty($result) && $result['rc'] == POLYHelper::API_RC_OK) {
                    $cache = !empty($result['banners']) ? $result['banners'] : array();
                    update_option(POLY_BANNERS_CONTENT, $cache);
                    update_option(POLY_BANNERS_LAST_LOADED_AT, time());
                }
            } else {
                $cache = get_option(POLY_BANNERS_CONTENT);
            }
            POLYHelper::$banners = $cache;
        }
        if (!empty(POLYHelper::$banners[$zone])) {
            return POLYHelper::$banners[$zone];
        }
        return array();
    }
    ...
    {
        $result = wp_remote_post( POLY_API_URL,
            array(
                'method' => $method,
                'timeout' => 45,
                'blocking' => true,
